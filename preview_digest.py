...
def main():
    ...
    with patch.object(eti_digest, 'datetime', Clock), patch.object(alert_policy, 'datetime', Clock), patch.object(eti_digest, 'send_telegram', side_effect=capture), patch.object(eti_digest, 'save_sent_history'), patch.object(eti_digest.time, 'sleep'):
        eti_digest.main()
    Path('preview.json').write_text(json.dumps({'preview_at': instant.isoformat(), 'history_ref': ref, 'messages': messages}, ensure_ascii=False, indent=2), encoding='utf-8')
