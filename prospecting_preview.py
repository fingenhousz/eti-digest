    ...
    for label, query in EXTRA_QUERIES:
        ...
        try:
            ...
            for entry in feed.entries[:60]:
                ...
                sources[f'R{len(sources)}'] = {'title': title, 'text': title + ' ' + summary,
...
def validate(message, sources, reject_ungrounded=False):
    ...
    for row in rows:
        ...
        if not company_key(row['company']) or company_key(row['company']) not in company_key(source):
            if reject_ungrounded:
                print('Fiche écartée : entreprise non établie dans la source citée')
                continue
    ...
    if rows and not accepted:
        raise ValueError('Aucune fiche ne passe les contrôles de provenance')
    # The source title is quoted by the renderer, not generated by the model.
    return [{**r, 'fact_quote': sources[r['source_id']].get('title', sources[r['source_id']]['text'])} for r in accepted]
...
def select(client, sources, exclusions):
    prompt = OFFER_PROMPT + '\n' + (
        ...
        'company doit reprendre exactement le nom présent dans text, sans ajouter de description ni de filiale supposée. '
    ...
    for attempt in range(2):
        ...
        try:
            return [r for r in validate(message, sources, reject_ungrounded=attempt == 1) if not company_keys(r['company']) & excluded]
        except ValueError as error:
            print('Validation de sélection : ' + str(error))
            prompt += '\nLa tentative précédente a échoué : ' + str(error) + '. Vérifie chaque champ, notamment le nom de l’entreprise dans text.'
